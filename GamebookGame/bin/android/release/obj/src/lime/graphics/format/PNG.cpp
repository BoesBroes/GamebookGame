...
bool decodeData = __o_decodeData.Default(true);
{
		Dynamic tmp;
		{
			...
			Dynamic buffer = tmp2;
			tmp = ::lime::_backend::native::NativeCFFI_obj::cffi_lime_png_decode_bytes.call(hx::DynamicPtr(bytes),decodeData,hx::DynamicPtr(buffer));
		}
		return tmp;
		return null();
	}
...
bool decodeData = __o_decodeData.Default(true);
{
		Dynamic tmp;
		{
			...
			::lime::graphics::ImageBuffer tmp2 = ::lime::graphics::ImageBuffer_obj::__new(tmp1,null(),null(),null(),null());
			Dynamic buffer = tmp2;
			tmp = ::lime::_backend::native::NativeCFFI_obj::cffi_lime_png_decode_file.call(path,decodeData,hx::DynamicPtr(buffer));
		}
		return tmp;
		return null();
	}
...
::haxe::io::Bytes PNG_obj::encode( ::lime::graphics::Image image){
	...
	if ((tmp3)){
		Dynamic tmp4;
		{
			::haxe::io::Bytes tmp5 = ::haxe::io::Bytes_obj::alloc((int)0);
			Dynamic bytes = tmp5;
			tmp4 = ::lime::_backend::native::NativeCFFI_obj::cffi_lime_image_encode.call(hx::DynamicPtr(image->buffer),(int)0,(int)0,hx::DynamicPtr(bytes));
		}
		return tmp4;
	}
	return null();
