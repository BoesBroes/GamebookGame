...
{
{
	...
	this->window = window;
	::lime::_backend::native::NativeRenderer tmp = ::lime::_backend::native::NativeRenderer_obj::__new(hx::ObjectPtr<OBJ_>(this));
	this->backend = tmp;
...
{
		::lime::_backend::native::NativeRenderer tmp = this->backend;
		tmp->create();
	}
return null();
}
...
::lime::graphics::Image Renderer_obj::readPixels( ::lime::math::Rectangle rect){
	::lime::_backend::native::NativeRenderer tmp = this->backend;
	::lime::math::Rectangle tmp1 = rect;
	::lime::graphics::Image tmp2 = tmp->readPixels(tmp1);
	return tmp2;
}
