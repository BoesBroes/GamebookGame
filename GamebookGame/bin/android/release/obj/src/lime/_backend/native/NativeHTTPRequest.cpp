...
Void NativeHTTPRequest_obj::loadFile( ::String path){
{
		...
		::String tmp2 = path;
		bool tmp3 = ::StringTools_obj::startsWith(tmp2,HX_HCSTRING("~/","\xf1","\x6d","\x00","\x00"));
		if ((tmp3)){
			::String tmp4 = ::Sys_obj::getEnv(HX_HCSTRING("HOME","\x1f","\x92","\xd3","\x2f"));
			::String tmp5 = (tmp4 + HX_HCSTRING("/","\x2f","\x00","\x00","\x00"));
			::String tmp6 = path.substr((int)2,null());
