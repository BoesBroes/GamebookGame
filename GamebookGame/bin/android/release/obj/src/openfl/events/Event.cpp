...
::openfl::events::Event Event_obj::clone( ){
	...
	bool tmp1 = this->bubbles;
	bool tmp2 = this->cancelable;
	::openfl::events::Event tmp3 = ::openfl::events::Event_obj::__new(tmp,tmp1,tmp2);
	::openfl::events::Event event = tmp3;
	int tmp4 = this->eventPhase;
	event->eventPhase = tmp4;
	::openfl::events::IEventDispatcher tmp5 = this->target;
	event->target = tmp5;
	::openfl::events::IEventDispatcher tmp6 = this->currentTarget;
	event->currentTarget = tmp6;
	::openfl::events::Event tmp7 = event;
	return tmp7;
}
...
::String Event_obj::formatToString( ::String className,::String p1,::String p2,::String p3,::String p4,::String p5){
	Array< ::String > parameters = Array_obj< ::String >::__new();
	bool tmp = (p1 != null());
	if ((tmp)){
		::String tmp1 = p1;
	...
	Dynamic tmp5 = this->__formatToString_dyn();
	Dynamic tmp6 = ::Reflect_obj::callMethod(hx::ObjectPtr<OBJ_>(this),tmp5,cpp::ArrayBase_obj::__new().Add(className).Add(parameters));
	return tmp6;
}
...
bool Event_obj::isDefaultPrevented( ){
	bool tmp = this->__preventDefault;
	return tmp;
}
...
::String Event_obj::toString( ){
	::String tmp = this->__formatToString(HX_HCSTRING("Event","\xfa","\x37","\xf2","\x08"),Array_obj< ::String >::__new().Add(HX_HCSTRING("type","\xba","\xf2","\x08","\x4d")).Add(HX_HCSTRING("bubbles","\x67","\xbb","\x56","\x61")).Add(HX_HCSTRING("cancelable","\x14","\xa0","\x79","\xc4")));
	return tmp;
}
...
::String Event_obj::__formatToString( ::String className,Array< ::String > parameters){
	::String tmp = (HX_HCSTRING("[","\x5b","\x00","\x00","\x00") + className);
	::String output = tmp;
	Dynamic arg = null();
	{
		int _g = (int)0;
		while((true)){
			bool tmp1 = (_g < parameters->length);
			bool tmp2 = !(tmp1);
			if ((tmp2)){
				break;
			}
			::String tmp3 = parameters->__get(_g);
			::String param = tmp3;
			++(_g);
			::String tmp4 = param;
			Dynamic tmp5 = ::Reflect_obj::field(hx::ObjectPtr<OBJ_>(this),tmp4);
			arg = tmp5;
			Dynamic tmp6 = arg;
			bool tmp7 = ::Std_obj::is(tmp6,hx::ClassOf< ::String >());
			if ((tmp7)){
				::String tmp8 = (HX_HCSTRING(" ","\x20","\x00","\x00","\x00") + param);
				::String tmp9 = (tmp8 + HX_HCSTRING("=\"","\x45","\x35","\x00","\x00"));
				Dynamic tmp10 = arg;
				::String tmp11 = ::Std_obj::string(tmp10);
				::String tmp12 = (tmp9 + tmp11);
				::String tmp13 = (tmp12 + HX_HCSTRING("\"","\x22","\x00","\x00","\x00"));
				hx::AddEq(output,tmp13);
			}
			else{
				::String tmp8 = (HX_HCSTRING(" ","\x20","\x00","\x00","\x00") + param);
				::String tmp9 = (tmp8 + HX_HCSTRING("=","\x3d","\x00","\x00","\x00"));
				Dynamic tmp10 = arg;
				::String tmp11 = ::Std_obj::string(tmp10);
				::String tmp12 = (tmp9 + tmp11);
...
Dynamic Event_obj::__SetField(const ::String &inName,const Dynamic &inValue,hx::PropertyAccess inCallProp)
{
	...
	case 4:
		...
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"target") ) { target=inValue.Cast< ::openfl::events::IEventDispatcher >(); return inValue; }
		break;
	case 7:
	...
	case 12:
		...
		break;
	case 13:
		if (HX_FIELD_EQ(inName,"currentTarget") ) { currentTarget=inValue.Cast< ::openfl::events::IEventDispatcher >(); return inValue; }
		break;
	case 15:
